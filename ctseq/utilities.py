...
def naturalSort(mylist):
    ...
    convert = lambda text: int(text) if text.isdigit() else text.lower() # converts to lowercase
    alphanum_key = lambda key: [ convert(c) for c in re.split('([0-9]+)', key) ] # separating numbers from the letters
    return sorted(mylist, key = alphanum_key) # sorts number by key
    # sorts numerically lowest to highest when you have letters in front of numbers
