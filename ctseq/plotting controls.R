...
# grabbing the controls we want to plot
controlSampleStats <- sampleStats[c(2,1),]
controlTotalMol <- totalMolFile[,c(2,1)]
controlMethMol <- methMolFile[,c(2,1)]
controlMethRatio <- methRatioFile[,c(2,1)]
...
colNamesContTotalMol <- colnames(controlTotalMol)
colNamesContTotalMol <- c("Locus",colNamesContTotalMol) # adds locus as a column name
controlTotalMol$Locus <- rownames(controlTotalMol)
controlTotalMol <- controlTotalMol[,colNamesContTotalMol] # locus is now the first column
rownames(controlTotalMol) = NULL # takes out the loci as being the row names, and they are now just 1,2,3,4, etc
...
meltedMol <- melt(controlTotalMol)
...
rownames(controlTotalMol) <- controlTotalMol$Locus
controlTotalMol$Locus=NULL
...
pheatmap(log2(controlTotalMol[fragOrder_heatmaps,]+1),
...
pheatmap(log2(controlMethMol[fragOrder_heatmaps,]+1), 
...
pheatmap(controlMethRatio[fragOrder_heatmaps,], 
