...
def run(args):
    ...
    runType=args.umiType # 'separate' if the UMIs are contained in a separate fastq file.'inline' if UMIs are already included in the forward and reverse read fastq files. 
    umiLength=int(args.umiLength) # Length of the UMI sequence
    forwardExt=args.forwardExt # Extension of the fastq file containing forward reads. Include .gz if files are compressed. 
    reverseExt=args.reverseExt # Extension of the fastq file containing reverse reads. Include .gz if files are compressed.
    umiExt=args.umiExt # Extension of the fastq file containing UMIs. Include .gz if files are compressed. Required if UMIs are in a separate fastq file from forward and reverse reads. 
    ...
    # Makes sure all the files needed are in your current directory
    ##############################################
    print('\n**************')
    ...
    for forwardFileName in allForwardFiles:
        ...
        print('****')
        if runType=='separate': # if the UMIs are separate from the forward and reverse reads
        ...
        elif runType=='inline': # if UMIs are already included in the forward and reverse read fastq files
